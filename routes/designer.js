...
router.get("/add-product", pHR, (req, res, next) => {
  UserModel.find({ role: "Designer" }).then((designers) => {
    UserModel.find().then((users) => {
      SerieModel.find()
        .then((series) => {
          res.render("./../views/users/1head/2hdaddnewdesign.hbs", {
            designers,
            users,
            series,
          });
...
router.post("/add-product", fileUploader.single("image"), (req, res, next) => {
  ...
  ProductModel.create({
    ...
    .then(async (product) => {
      ...
      // push editors to array of editors
      /*
      const editorPromise = await updateEditorsOfProduct(
        product._id,
        editorsToPush
      );
...
router.get("/series", pHR, (req, res, next) => {
  SerieModel.find()
    .then((series) =>
      res.render("./../views/users/1head/3hdseries.hbs", { series })
    )
    .catch((err) => next(err));
});
...
router.get("/series/:id", pHR, (req, res, next) => {
  SerieModel.findByIdAndDelete(req.params.id).then((serie) => {
...
router.get("/manage-rights", pHR, (req, res, next) => {
  UserModel.find().then((users) => {
    ProductModel.find()
      .populate("editors")
      .then((products) => {
        res.render("./../views/users/1head/4managerights.hbs", {
          users,
          products,
        });
      })
...
router.get("/manage-rights/:id", pHR, (req, res, next) => {
  UserModel.findById(req.params.id)
    .then((user) =>
      res.render("./../views/users/1head/6hdedituser.hbs", { user })
    )
    .catch((err) => next(err));
});
...
router.post("/manage-rights/:id", (req, res, next) => {
  ...
  UserModel.findByIdAndUpdate(
    ...
    const removeUserIdFromEditors = removeUserIdFromProductsEditors(
      req.params.id
    );
  ...
  ProductModel.findById(req.params.id)
    ...
    .then((product) => {
      UserModel.find().then((users) => {
        SerieModel.find()
          .then((series) => {
            res.render("./../views/users/1head/5hdeditproduct.hbs", {
              product,
              users,
              series,
            });
...
router.post(
  ...
  (req, res, next) => {
    ...
    ProductModel.findByIdAndUpdate(
      ...
      // push editors to array of editors
      const editorPromise = updateEditorsOfProduct(
        req.params.id,
        editorsToPush
      );
      editorPromise.then((editedProductWithEditors) => {
        ...
        const singleEditors = editedProductWithEditors.editors.filter(
          (ed, i) => editedProductWithEditors.editors.indexOf(ed) === i
        );
        console.log("SE", singleEditors);
        // delete all editors from product
        const deleteEditorsPromise = deleteEditors(
          req.params.id,
          editedProductWithEditors
        );
        deleteEditorsPromise.then((productWithNoEditors) => {
          console.log("pwne", productWithNoEditors);
          const singleEditorsPromise = includeSingleEditors(
            req.params.id,
            singleEditors
          );
          ...
          singleEditorsPromise
            .then((productWithSingleEditors) => {
              ...
              if (req.file) {
                ...
                const imagePromise = addImageOfProduct(
                  req.params.id,
                  req.file.path
                );
                imagePromise
...
// update role to editor when editor is added for product
...
function updateEditorsOfProduct(id, editorsIds) {
  const editorPromise = ProductModel.findByIdAndUpdate(
    id,
    { $push: { editors: editorsIds } },
    { new: true }
  );
...
function deleteEditors(id, product) {
  const deleteEditors = ProductModel.findByIdAndUpdate(
    id,
    { $pullAll: { editors: product.editors } },
    { new: true }
  );
...
function includeSingleEditors(id, singleEditors) {
  const singleEditorsPromise = ProductModel.findByIdAndUpdate(id, {
...
function addImageOfProduct(id, imagePath) {
  const imagePromise = ProductModel.findByIdAndUpdate(
    id,
...
function removeUserIdFromProductsEditors(userId) {
  const removeUserIdPromise = ProductModel.updateMany(
    { editors: userId },
    { $pull: { editors: userId } },
    { new: true }
  );
